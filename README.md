...
目前 v2 已包含病人建立／修改／搜尋／排序／安全刪除與重載、完整 25 區塊／194 題／16 科別 typed catalog、全部 ROS/PE 題型、Admission/ADL、PMH、待辦、全域與區塊備註、所有專用／自訂組套，以及臨床摘要匯出／列印。Google 模式已完成 Identity Services 登入、session-only token、帳號隔離快取、離線開啟、三方合併、衝突備份與 Drive `appDataFolder` adapter；正式入口仍須通過剩餘 parity gate 才會切換。
