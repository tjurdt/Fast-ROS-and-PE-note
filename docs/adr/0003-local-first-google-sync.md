...
同步演算法與 Google API、React UI 分離，可用記憶體 fake 完整測試。App 只依賴 `SyncCapablePatientRepository`，未來組裝 OAuth 或更換遠端供應者不需修改病人 domain 與 feature。代價是同步會保留額外 base snapshot，且衝突後可能需要第二輪同步；這是保護輸入與可恢復性的刻意選擇。
