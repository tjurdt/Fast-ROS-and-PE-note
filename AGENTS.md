...
- `src/domain/clinical/catalog.generated.json` is generated from the frozen legacy oracle, including specialty focus, CN panels, and neurological widget options. Do not hand-edit it; run `npm run sync:clinical-catalog` and keep `check:clinical-catalog` passing.
